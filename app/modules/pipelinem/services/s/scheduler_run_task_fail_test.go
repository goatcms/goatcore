...
func TestSchedulerRunFail(t *testing.T) {
	t.Parallel()
	var (
	...
	task2Executor := NewMockedFailScriptExecutor()
	...
	if task1Executor.Runned() {
		t.Errorf("Task 1 shouldn't was runned (because it is dependent on failed task 2)")
	}
	if !task2Executor.Runned() {
		t.Errorf("Task2 should be runned")
	}
	if deploy1Executor.Runned() {
		t.Errorf("triggerDeployTask1 deploy task shouldn't be runned (because main task failed)")
	}
	if deploy2Executor.Runned() {
		t.Errorf("triggerDeployTask2 deploy task shouldn't be runned (because main task failed)")
	}
