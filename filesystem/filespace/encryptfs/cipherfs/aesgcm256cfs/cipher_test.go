...
func TestEncryptDecryptStory(t *testing.T) {
	...
	if encrypted, err = NewCipher().Encrypt(key, expected); err != nil {
		t.Error(err)
		return
	}
	if decrypted, err = NewCipher().Decrypt(key, encrypted); err != nil {
		t.Error(err)
		return
	}
...
func TestEncryptDecryptStreamStory(t *testing.T) {
	...
	if writer, err = NewCipher().EncryptWriter(key, buff); err != nil {
		t.Error(err)
		return
	}
	...
	if reader, err = NewCipher().DecryptReader(key, buff); err != nil {
		t.Error(err)
		return
	}
